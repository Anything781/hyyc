...
with tf.Session(config=config) as sess:
    ...
    #初始化模型
    allresult = np.zeros((500))  # 所有的result
    gradmax = np.zeros((500))
    ...
    step = 2*np.exp(-2)     #控制每次迭代步长，乘上梯度，就是直接加上梯度*step的值
    k = 15              #第15维的33:36，即97年的10-12月(72,24,6)
    tmp = ftX.copy()   #负值新的数据，避免影响原图值
    tmp1 = ftX.copy()
    ftX0 = tmp[k]
    #迭代500次
    for i in range(10):
        # grads是列表 （1,1,72,24,6） 加【0】取后四维
        gradients = tf.gradients(py_x,X)[0]         #py_x是输出值，l3b是最后一层特征图
        grads, result = sess.run([gradients,py_x],feed_dict={X: ftX0.reshape(1, xdim, ydim, zdim),
                                       p_keep_conv: 1.0, p_keep_hidden: 1.0})
        grads = grads.reshape(72,24,6)

        result = result.reshape(1)
        '''#对梯度归一化处理,原step=1或0.1,使得梯度更连贯
        grads /= (np.std(grads)+np.exp(-8))
        #将正规化处理后的梯度应用在输入数据上，'''

        # 对梯度处理，绝对值求平均然后用学习率step除以这个均值,得到可用的学习率
        grads_abs = np.abs(grads)
        grads_abs = np.mean(grads_abs)
        usestep = step / (grads_abs + np.exp(-6))  # 梯度大步长小，梯度小，步长大
        usestep_tmp = usestep.copy()


        if grads.max() != 0:
            ftX0_copy = ftX0.copy()
            ftX0 += grads * usestep
            print('grads not 0 max:',grads.max(),' ftX0 max: ',ftX0.max())

            result_tmp = sess.run(py_x, feed_dict={X: ftX0.reshape(1, xdim, ydim, zdim),
                                             p_keep_conv: 1.0, p_keep_hidden: 1.0})
            change_ftX0 = ftX0_copy - ftX0
            while change_ftX0.max() == 0: #输入更新量太小，10倍增大步长至有输入变化
                usestep *= 10
                ftX0 += grads * usestep
                '''result_tmp = sess.run(py_x, feed_dict={X: ftX0.reshape(1, xdim, ydim, zdim),
                                         p_keep_conv: 1.0, p_keep_hidden: 1.0})
                result_tmp = result_tmp.reshape(1)
                change_result = result_tmp - result'''
                change_ftX0 = ftX0_copy - ftX0
                print('ftX0 not 0 is change: ',np.sum(ftX0-ftX0_copy))
                print('grads*usestep.max: ',(grads*usestep).max())
                print('ftX0 max: ',ftX0.max())
                if (grads*usestep).max() >1000:#此时更新量过大，break
                    break
                #输入有变化后，ftx0也保留了下来

        else:
            '''ftX1 = ftX0 + grads * usestep

            grads = sess.run(gradients, feed_dict={X: ftX1.reshape(1, xdim, ydim, zdim),
                                              p_keep_conv: 1.0, p_keep_hidden: 1.0})
            grads_copy = grads.copy()'''
            while grads.max() == 0:
                usestep *= 10
                #此时梯度仍为0，对梯度加上一个非常小的数,避免ftX0不更新，没法求增大步长后的梯度
                ftX0 += (grads+np.exp(-7)) * usestep
                grads = sess.run(gradients, feed_dict={X: ftX0.reshape(1, xdim, ydim, zdim),
                                             p_keep_conv: 1.0, p_keep_hidden: 1.0})
                grads = grads.reshape(72, 24, 6)
                print('grads max: ',grads.max(),'usestep: ',usestep)

                if usestep > 1000000000:
                    break

            if usestep > 1000000000:
                #此时梯度仍为0，对梯度加上一个非常小的数，使得输入有略微更新，这里使用原来的步长
                ftX0 += (grads+np.exp(-7)) * usestep_tmp
            else:
                #增大步长之后梯度不为0了，然后利用此时的梯度和此时的步长
                ftX0 += grads * usestep
                print('ftX0+ max:',(grads * usestep).max())

        print('time: ', i, ' result: ', result)
        allresult[i] = result
        gradmax[i] = grads.max()

    ftX0 = np.swapaxes(ftX0,0,2) #变为（6,24,72）

tmp1[15].astype('float32').tofile('/mnt/deepdream/971mon12deepdreaminput.gdat')
ftX0.astype('float32').tofile('/mnt/deepdream/971mon12deepdreaminputchange.gdat')
allresult.astype('float32').tofile('/mnt/deepdream/97allresult.gdat')
gradmax.astype('float32').tofile('/mnt/deepdream/971mon12gradmax.gdat')
