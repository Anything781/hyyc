...
beishu = multiple * 0.01   #乘0.1还是0.001取决于.csh文件中bei步长是多少
...
with tf.device('/cpu:0'):  # Using CPU
    ...
    # 修改数据              1mon12
    m = np.fromfile('/mnt/retraindisturbto0/4x4all6to072x24.gdat', dtype=np.float32)
    m = m.reshape(72, 24)
...
with tf.Session(config=config) as sess:
    saver.restore(sess,'/mnt/document/lmont/cmip/chlist/0.'+str(multiple)+'/model.ckpt')
    ...
    save_path = saver.save(sess,'/mnt/document/lmont/transfer/chlist/0.'+str(multiple)+'/model.ckpt')
                                #mnt/retrain0.3250.675/1mon1/transfer/c30h30/0.1
