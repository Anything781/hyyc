...
with tf.Session(config=config) as sess:
    ...
    #初始化模型
    en_mean = np.zeros((36, xdim2, ydim2))  # tdim是第一维的个数(36,18,6)
    ...
    for k in range(36):
        ...
        grads_mean = np.mean(grads,axis=(0,1,2))  # M 个
        ...
        heatmap = np.maximum(heatmap,0)  #相当于relu
        # heatmap /= np.max(heatmap)  #除以其中的最大值,使数值在0~1范围方便后来*255 resize？

        en_mean[k,:,:] = heatmap    #每一个EN的heatmap

    # save heatmap (4-byte binary)
en_mean.astype('float32').tofile('/mnt/heatmapC50H50EN10/gradcamheatmap.gdat')
