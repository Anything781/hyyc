...
with tf.device('/cpu:0'):  # Using CPU
    ...
    m = np.fromfile('/mnt/cnngrad/lmont/lmontgradCHmeandel0.gdat', dtype=np.float32)
    m = m.reshape(72, 24, 6)
    m1 = np.swapaxes(m, 0, 2)  # 交换0, 2维，（72,24,6）——>（6,24,72）符合原数据类型
    norm = np.zeros((6, 24, 72))
    for c in range(6):
        norm[c] = (m1[c] - m1[c].min()) / (m1[c].max() - m1[c].min())  # (24,72) 最大最小归一化
        print('channel ', c, ' norm min: ', norm.min(), 'channel ', c, ' norm max: ', norm.max())
