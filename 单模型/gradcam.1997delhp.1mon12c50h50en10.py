...
aa = np.minimum(m1[15],0)   #只保留负值，即删掉正热图值部分
...
#此处是只留下了负值部分，就是删掉了正热图值部分
b1 = b<0 #提取热图值小于0的数据位置
#不能取 =0，因为上边已经把大于0部分设置为0了，再取等于0还是保留了所有的数据
for ii in range(6):
    inpv1[15,ii,:,:] *= b1
...
with tf.Session(config=config) as sess:
    ...
    result.astype('float32').tofile('/mnt/heatmapC50H50EN10/'+'gradcam.delhpresult.gdat')
