...
with tf.Session(config=config) as sess:
    ...
    for i in range(9):
        ...
        #改用组合形式，0.+str（i+1）
        saver.restore(sess,'/mnt/grad/retrain1mon12c50h50en10/transfer/0.'+str(i+1)+'/model.ckpt')
        ...
        tmp = tmp.reshape(36)
        result[i,:] = tmp
    result.astype('float32').tofile('/mnt/grad/retrain1mon12c50h50en10/transfer/result.gdat')
