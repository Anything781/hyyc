...
with tf.Session(config=config) as sess:
    ...
    #初始化模型
    grad100 = np.zeros((10,100, 72, 24))  # test_size是第一维的个数(100,72,24)
    grad2961 = np.zeros((10,2961, 72, 24))
    for j in range(10):
        saver.restore(sess, '/mnt/output/nino34_1month_12_Transfer_20/' + CH_list + '/EN' + str(j + 1) + '/model.ckpt')
        ...
        grads_x_mean = np.mean(grads_x, axis=3)  # (100,72,24)
        ...
        grads_x_cmip_mean = np.mean(grads_x_cmip, axis=3)  # (2961,72,24)

        grad2961[j,:,:,:] = grads_x_cmip_mean

        print(time.strftime("%Y-%m-%d-%H_%M_%S", time.localtime()))

    tmp = np.zeros((2,72,24))
    tmp[0,:,:] = np.mean(grad100,axis=(0,1))
    tmp[1,:,:] = np.mean(grad2961,axis=(0,1))
    grad_all_mean = np.mean(tmp,axis=0)
    ...
    grad3.astype('float32').tofile('/mnt/grad/1mon12'+CH_list+'/gradmeandel0.gdat')

    grad100.astype('float32').tofile('/mnt/grad/1mon12' + CH_list + '/grad100.gdat')
    grad2961.astype('float32').tofile('/mnt/grad/1mon12' + CH_list + '/grad2961.gdat')
    grad_all_mean.astype('float32').tofile('/mnt/grad/1mon12' + CH_list + '/gradmean.gdat')
    print('100,2961 min(0 or not):',tmp[0].min(),tmp[1].min())
print(time.strftime("%Y-%m-%d-%H_%M_%S", time.localtime()))
