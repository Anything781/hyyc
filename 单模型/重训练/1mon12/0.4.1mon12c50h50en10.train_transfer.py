...
batch_size = 20
num_convf = 50   #convfilter
num_hiddf = 50   #hiddfilter
xdim = 72
ydim = 24
zdim = 6
xdim2 = int(xdim/4)
ydim2 = int(ydim/4)
sample_size = 100   #SAMSIZ
tot_size = 100      #TOTSIZ
conv_drop = 1.0     #CDRP
hidd_drop = 1.0     #HDRP
...
with tf.device('/cpu:0'): # Using CPU
  ...
  # 修改数据
  m = np.fromfile('/mnt/grad/gradmean1mon12c50h50en10del0.gdat', dtype=np.float32)
  m = m.reshape(72, 24)
  m1 = np.swapaxes(m, 0, 1)  # 交换0, 1维，（72,24）——>（24,72）符合原数据类型
  condition = m1.max() * beishu  # 保留数据的条件
  aa = m1 > condition           #aa是筛选出来符合条件的数据位置
...
with tf.Session(config=config) as sess:
    saver.restore(sess,'/mnt/grad/retrain1mon12c50h50en10/cmip/'+str(beishu)+'/model.ckpt')
    ...
    save_path = saver.save(sess,'/mnt/grad/retrain1mon12c50h50en10/transfer/'+str(beishu)+'/model.ckpt')
                        #output/nino34_'$LEAD'month_'$TMON'_Transfer_20/C30H30/ENmember
    print(condition)
