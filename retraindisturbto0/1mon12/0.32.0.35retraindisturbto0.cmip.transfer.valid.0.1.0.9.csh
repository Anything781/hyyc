...
setenv RET 'retraindisturbto0'					#重训练文件夹

setenv MOD 're1mon12'			#重训练的模型     如 1mon1
...
@ bei = 32
while ($bei <= 35 )									#防止因加小数0.5出现精度问题，改为加50，cmip和transfer乘0.1改为0.001即可
...
cp -f $HHH/$RET/$MOD/retrain.cmip.sample .					#这里要提前把训练代码放在retraindisturbto0文件夹下
...
cp -f $HHH/$RET/$MOD/retrain.transfer.sample .				#这里要提前把训练代码放在retraindisturbto0文件夹下
...
cp -f $HHH/$RET/$MOD/retrain.valid.sample .				 #这里要提前把训练代码放在retraindisturbto0文件夹下
...
cp -f $HHH/$RET/$MOD/retrain.validmean.sample .			 #这里要提前把训练代码放在retraindisturbto0文件夹下
