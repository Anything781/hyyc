...
inpv3 = inpv1.copy()  #复制一个初始输入数据来求标准差
...
with tf.Session(config=config) as sess:
    ...
    for j in range(10):
        ...
        grads_x_mean0 = np.mean(grads_x, axis=-1)  # (100，72,24)
        std100 = np.std(ftX,axis=-1) #(100,72,24)
        grads_x_mean = grads_x_mean0 * std100
        ...
        grads_x_cmip_mean0 = np.mean(grads_x_cmip, axis=-1)  # (2961,72,24)
        std2961 = np.std(ftX_cmip,axis=-1)#(2961,72,24)
        grads_x_cmip_mean = grads_x_cmip_mean0 * std2961

        grad2961[j,:,:,:] = grads_x_cmip_mean   #(10,2961,72,24)

    tmp = np.zeros((2,72,24))
    tmp[0] = np.mean(grad100,axis=(0,1))
    tmp[1] = np.mean(grad2961,axis=(0,1))
    gradstd_all_mean = np.mean(tmp,axis=0)              #(72,24)

    gradstd_all_mean.astype('float32').tofile('/mnt/cnngradstd/lmont/chlistgradstdmean.gdat')
