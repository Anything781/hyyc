...
ipth2 = 'E:/hyyc/对比实验/disturbinput/all6to0/1mon12/0.32-0.351mon12/4x4heatmap'
'''1x1 (36,24,72)   2x2 (36,12,36)   4x4 (36,6,18) '''

#heat_each = np.zeros((36,18,6), dtype=np.float32)

f = open(ipth1+'/'+'4x4CHmeanall6to0.gdat','r')

a = np.fromfile(f, dtype=np.float32).reshape(36,6,18)
a = np.mean(a,axis=0)
print(a.shape)              #（6,18）
...
for i in range(32,36,1):
    ...
    space = '                                                              '
    plt.title('1month12 Occlusion-4x4 0.'+str(i)+space+'[El Niño Case]',fontsize=8, y=0.962,x=0.5)
    ...
    plt.savefig(ipth2 +'/4x4occlusion0.'+str(i)+'.jpg',dpi = 500)#默认dpi=100 dpi=155时突然变方块
    plt.close()
