...
ipth = 'E:/hyyc/retrain0.325-0.675/'+model+'/retrainresult/'    #保存重训练结果文件夹
ipth1 = 'E:/hyyc/retrain0.325-0.675/allretrainresult/'              #重训练结果文件夹
...
retrain = retrain.reshape(8,36)
'''0.325-0.675是（8,36）,0.05步长0.325开始到0.675'''
...
for ii in range(8):
    ...
    aa =int((ii * 10 +65)*5) #取整
    plt.title(model+' 0.'+str(aa)+' retrain result nino3.4 ', fontsize=8)
    plt.xlabel('Year', fontsize=7)
    plt.ylabel('Nino3.4', fontsize=7)

    plt.savefig(ipth+'retrain0.'+str(aa)+'.jpg', dpi=500)
    plt.close()
    print(model)