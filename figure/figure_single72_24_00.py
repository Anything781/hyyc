...
for ii in range(23):
    for j in range(12):
        ...
        for i in range(20,75,5):   #0.05倍步长画图
            '''按条件保留数据,保留倍数以上的数据'''
            ...
            '''原来extent=[0,380,60,-55]，为了显示出被热图覆盖的四周的黑框线，
                需要把热图画图区域适当缩小一点点'''
            ...
            space = '                                                                '
            plt.title(str(lead)+'month'+str(target)+' P.T.P.V 0.'+str(i)+'0'+space+' [El Niño Heatmap]',fontsize=8, y=0.965,x=0.5)
            ...
            plt.savefig(ipth1 +'0.'+str(i)+'0gradstd.jpg',dpi=200)#默认dpi=100 dpi=155时突然变方块
