...
ipth1 = 'E:/hyyc/对比实验/disturbinput/all6to0/'+lmont+'/0.32-0.351mon12/4x4retrain/'      #重训练结果文件夹
ipth2 = 'E:/hyyc/cnnresult/'                                         #原cnn预测结果文件夹



#retrain result (9,36) 0.1~0.9 gradient
retrain = open(ipth1 +'0.32.0.35re'+lmont+'CHmeanresult.gdat', 'r')
retrain = np.fromfile(retrain,dtype=np.float32)
retrain = retrain.reshape(4,36)
...
for ii in range(4):
    ...
    plt.title(lmont+' Occlusion-4x4 0.'+str(ii+32)+' retrain result nino3.4 ', fontsize=8)
    plt.xlabel('Year', fontsize=7)
    plt.ylabel('Nino3.4', fontsize=7)

    plt.savefig(ipth1+'retrain0.'+str(ii+32)+'.jpg', dpi=500)
    plt.close()
    #print(model)