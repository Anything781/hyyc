...
for a in range(12):
    ...
    ipth = 'E:/hyyc/retrain/retrainnoleftrighttop/'+model+'/'         #保存重训练相关性系数文件夹
    ipth1 = 'E:/hyyc/retrain/retrainnoleftrighttop/allretrainresult/'   #重训练结果文件夹
    ...
    '''0.2-0.7是（11,36），从0.2到0.7，0.05步长增加'''
    '''0.1-0.9是（9,36）,0.1-0.7是（7,36）'''
    ...
    for ii in range(7):
        ...
        plt.xlim([0, 38])#x轴作图范围

        #plt.xlim([0, 102])
        plt.ylim([-3, 3.5])#y轴作图范围
        plt.xticks(np.arange(2, 39, 2), np.arange(1982, 2019, 2), fontsize=6.5)#x轴刻度内容范围
        #第一个参数是画图范围，第二个参数是画图数据
        ...
        plt.axhline(0, color='black', linewidth=0.5)#绘制水平参考线，第一个参数代表画水平线y轴位置

        plt.title(model+' 0.'+str(ii+1)+' retrain result nino3.4 ', fontsize=8)
        plt.xlabel('Year', fontsize=7)
        plt.ylabel('Nino3.4', fontsize=7)

        plt.savefig(ipth+'retrain0.'+str(ii+1)+'.jpg', dpi=500)
        plt.close()
        print(model)