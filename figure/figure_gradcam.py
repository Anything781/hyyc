...
ipth1 = 'E:/hyyc/对比实验/gradcam/1mon12/abs/heatmap'
#ipth2 = 'E:/hyyc/disturbinput/4x4'



# Open Heatmap of each case (1981-2016)  36年
#heat_each = np.zeros((36,18,6), dtype=np.float32)
f0 = open(ipth1+'/'+'allCHgradcammeanabs.gdat','r')  #allCHgradcammean(36,18,6)
heat_each = np.fromfile(f0, dtype=np.float32).reshape(36,18,6)
heat_each = np.mean(heat_each,axis=0) #取36年平均
...
for i in range(9):
    ...
    ext_heatmap = np.append(heat_each1,heat_each1[:,0:1],axis=1)
    ...
    space = '                                                                         '
    plt.title('1month12 gradcam 0.'+str(i+1)+space+'[El Niño Case]',fontsize=8, y=0.962,x=0.5)
    ...
    plt.savefig(ipth1 +'/gradcam0.'+str(i+1)+'.jpg',dpi = 500)#默认dpi=100 dpi=155时突然变方块
    plt.close()
